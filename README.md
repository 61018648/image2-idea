...
如果需要将本项目从纯前端个人工具改造成多用户、按次付费或套餐订阅的生图平台，可参考 [多用户付费生图平台商业化改造方案](docs/commercial-platform-plan.md)。
...
- `PLATFORM_API_URL`：平台托管后端地址。启用后，Nginx 会将同源 `/api/platform/...` 请求转发到该地址。
- `PLATFORM_API_TOKEN`：平台托管后端鉴权令牌，由 Nginx 在转发时注入为 `Authorization: Bearer ...`。
- `PLATFORM_GATEWAY_USER_ID`：平台网关注入的平台用户 ID。当前 MVP 可用固定用户 ID；接入真实登录后应替换为可信会话解析结果。
- `PLATFORM_DEV_MODE`：设为 `true` 时可跳过平台鉴权，仅用于本地调试。

> ⚠️ **安全警告**：开启 API 代理后，任何人都能将你的服务器作为代理来请求目标 API。建议仅在有访问控制（如 IP 白名单）或本地网络中开启。

> ⚠️ **平台网关警告**：`PLATFORM_API_URL` / `PLATFORM_API_TOKEN` 仅应由服务器环境变量提供，不要把它们写进前端配置或浏览器可访问的静态文件中。

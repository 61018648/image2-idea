...
function normalizeAvatarUrl(value: unknown): string | null | undefined {
  const avatarUrl = normalizeOptionalText(value, 3_000_000)
  if (!avatarUrl) return avatarUrl
  if (/^https?:\/\//i.test(avatarUrl) || avatarUrl.startsWith('/api/platform/assets/')) return avatarUrl

  const match = /^data:([^;,]+);base64,([\s\S]+)$/i.exec(avatarUrl)
  if (!match) throw new Error('Avatar must be an image data URL or URL')
  const mimeType = match[1].toLowerCase()
  if (!ALLOWED_AVATAR_MIME_TYPES.has(mimeType)) throw new Error('Avatar image type is not supported')
  if (estimateBase64Bytes(match[2]) > MAX_AVATAR_BYTES) throw new Error('Avatar image must be 2MB or smaller')
  return avatarUrl
}

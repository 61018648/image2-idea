...
export default function SettingsModal() {
  ...
  const activeProviderUsesApiUrl = activeProviderIsOpenAICompatible || activeProfile.provider === 'fal' || activeProfile.provider === 'platform'
  ...
  const defaultProviderOrder = ['openai', 'fal', 'platform', ...draft.customProviders.map(p => p.id)]
  ...
  const unorderedProviderOptions = [
    ...
    { label: '平台托管', value: 'platform', draggable: true },
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    const normalizedProfiles = nextDraft.profiles.map((profile) => {
      ...
      const shouldKeepEmptyBaseUrl = profile.provider !== 'fal' && (profile.provider === 'platform' || nextApiProxy) && !profile.baseUrl.trim()
  ...
  const handleProviderReorder = (sourceValue: string | number, targetValue: string | number, position: 'before' | 'after' | null) => {
    const currentOrder = draft.providerOrder || ['openai', 'fal', 'platform', ...draft.customProviders.map(p => p.id)]
        ...
        <div className="flex flex-1 min-h-0 flex-col sm:flex-row">
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'api' && (
              ...
              {activeProviderUsesApiUrl && (
                <label className="block">
                  ...
                  <input
                    ...
                    placeholder={activeProfile.provider === 'fal' ? DEFAULT_FAL_BASE_URL : activeProfile.provider === 'platform' ? DEFAULT_PLATFORM_BASE_URL || '同源 /api/platform' : DEFAULT_SETTINGS.baseUrl}
                  ...
                  <div data-selectable-text className="mt-1.5 min-h-[22px] flex items-center text-xs text-gray-500 dark:text-gray-500">
                    ...
                    ) : activeProfile.provider === 'platform' ? (
                      <span>留空时请求同源 <code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">/api/platform</code>；也可填写独立平台后端地址。</span>
                    ) : (
              ...
              {/* 5. API Key */}
              {activeProfile.provider === 'platform' ? (
                <div data-selectable-text className="rounded-xl border border-blue-100 bg-blue-50/70 px-3 py-2.5 text-xs text-blue-700 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-200">
                  平台托管模式不会在浏览器保存模型 API Key；模型密钥、扣费和上游请求应由平台后端处理。
                </div>
              ) : (
                <div className="block">
              ...
              <label className="block">
                ...
                <input
                  ...
                  type="text"
                  placeholder={activeProfile.provider === 'fal' ? DEFAULT_FAL_MODEL : activeProfile.provider === 'platform' ? DEFAULT_PLATFORM_MODEL : getDefaultModelForMode(activeProfile.apiMode ?? DEFAULT_SETTINGS.apiMode)}
                  className="w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                />
                <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                  {activeProfile.provider === 'fal' ? (
                    ...
                    <>平台托管模式下模型 ID 仅作为前端展示与任务记录，实际模型路由由平台后端决定。</>
            ...
            {activeTab === 'about' && (
              <div className="flex h-full min-h-[300px] flex-col items-center justify-center pb-8 px-6">
                ...
                */}
                <a
                  href="https://github.com/61018648/image2-idea"
                  target="_blank"
                ...
                <p className="mt-8 mb-6 max-w-[360px] text-center text-[13px] leading-relaxed text-gray-500 dark:text-gray-400">
                  本项目基于开源项目<a href="https://github.com/CookSleep/gpt_image_playground">GPT Image Playground</a>进行二次开发，如在使用过程中遇到问题请及时反馈，网站离不开每一位用户的使用、反馈、贡献与支持，感谢一路有你。
                </p>

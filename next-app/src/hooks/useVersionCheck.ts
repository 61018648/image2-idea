...
const VERSION_CHECK_ENABLED = clientEnv.VITE_ENABLE_VERSION_CHECK === 'true'
 ...
 * - 仅当最新 Release 版本高于当前 APP_VERSION 时提示。
...
export function useVersionCheck() {
  ...
  const [dismissed, setDismissed] = useState(() =>
    typeof sessionStorage !== 'undefined' && sessionStorage.getItem('version-dismissed') === 'true',
  )
  ...
  const dismiss = () => {
    setDismissed(true)
    if (typeof sessionStorage !== 'undefined') sessionStorage.setItem('version-dismissed', 'true')
  }

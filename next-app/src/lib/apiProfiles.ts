...
const RAW_DEFAULT_API_URL = readRuntimeEnv(clientEnv.VITE_DEFAULT_API_URL)
const DEFAULT_PROVIDER = readRuntimeEnv(clientEnv.VITE_DEFAULT_PROVIDER)
const DEFAULT_OPENAI_API_PROXY = readRuntimeEnv(clientEnv.VITE_API_PROXY_AVAILABLE) === 'true'
const DOCKER_DEPLOYMENT = readRuntimeEnv(clientEnv.VITE_DOCKER_DEPLOYMENT) === 'true'
...
export const DEFAULT_PLATFORM_MODEL = 'platform-default'
export const DEFAULT_OPENAI_PROFILE_ID = 'default-openai'
export const DEFAULT_API_TIMEOUT = 600

const BUILT_IN_PROVIDER_IDS = new Set<ApiProvider>(['openai', 'fal', 'platform'])
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  ...
  if (provider === 'platform') {
    return {
      ...profile,
      provider,
      baseUrl: savedDraft?.baseUrl ?? DEFAULT_PLATFORM_BASE_URL,
      apiKey: '',
      model: savedDraft?.model ?? DEFAULT_PLATFORM_MODEL,
      apiMode: 'images',
      codexCli: false,
      apiProxy: false,
...
function normalizeProviderDraft(input: unknown, provider: ApiProvider, customProviderIds: Set<string>): ApiProfileProviderDraft {
  ...
  const knownProvider = provider === 'fal' || provider === 'openai' || provider === 'platform' || customProviderIds.has(provider)
  ...
  return {
    baseUrl: provider === 'fal'
      ...
      : provider === 'platform'
      ? baseUrl?.trim().replace(/\/+$/, '') ?? DEFAULT_PLATFORM_BASE_URL
      : baseUrl,
    model,
    apiMode,
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  const provider: ApiProvider = rawProvider === 'fal' || rawProvider === 'platform' || customProviderIds.has(rawProvider) ? rawProvider : 'openai'
  ...
  const defaults = provider === 'fal'
    ...
    : provider === 'platform'
    ? createDefaultPlatformProfile(fallback)
  ...
  return {
    ...
    baseUrl: provider === 'fal'
      ...
      : rawBaseUrl,
    apiKey: provider === 'platform' ? '' : typeof record.apiKey === 'string' ? record.apiKey : defaults.apiKey,
...
export function getApiProviderLabel(settings: Partial<AppSettings> | unknown, provider: ApiProvider): string {
  ...
  if (provider === 'platform') return '平台托管'
...
export function getActiveApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile {
  ...
  const shouldApplyLegacyOverrides = !hasExplicitProfiles || (profile.provider === 'openai' && profile.id === DEFAULT_OPENAI_PROFILE_ID)
  if (!shouldApplyLegacyOverrides) return profile
...
export function validateApiProfile(profile: ApiProfile): string | null {
  ...
  if (profile.provider !== 'fal' && profile.provider !== 'platform' && !profile.baseUrl.trim() && !shouldUseApiProxy(profile.apiProxy)) return '缺少 API URL'
  if (profile.provider !== 'platform' && !profile.apiKey.trim()) return '缺少 API Key'

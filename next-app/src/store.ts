...
import { IMAGE_FETCH_CORS_HINT, MIME_MAP } from './lib/imageApiShared'
import { getFalErrorMessage, getFalQueuedImageResult } from './lib/falAiImageApi'
import { getCustomQueuedImageResult } from './lib/openaiCompatibleImageApi'
import { getPlatformGeneration } from './lib/platformGenerationApi'
import { normalizePlatformGenerationJob, PlatformGenerationPendingError } from './lib/platformImageApi'
...
import { toUserFacingGenerationError } from './lib/userFacingErrors'
...
const PLATFORM_RECOVERY_POLL_MS = 10_000
...
const platformRecoveryTimers = new Map<string, ReturnType<typeof setTimeout>>()
...
export function markInterruptedOpenAIRunningTasks(tasks: TaskRecord[], now = Date.now()) {
  ...
  const updatedTasks = tasks.map((task) => {
    if (!isRunningOpenAITask(task) || task.customTaskId || task.platformJobId) return task
...
function getPlatformRecoveryProfile(settings: AppSettings, task: TaskRecord) {
  const taskProfile = getTaskApiProfile(settings, task)
  if (taskProfile?.provider === 'platform') return taskProfile
  const fallback = normalizeSettings(settings).profiles.find((profile) => profile.provider === 'platform')
  return fallback ?? createDefaultPlatformProfile({ id: task.apiProfileId || 'default-platform' })
}
...
function clearPlatformRecoveryTimer(taskId: string) {
  const timer = platformRecoveryTimers.get(taskId)
  if (timer) clearTimeout(timer)
  platformRecoveryTimers.delete(taskId)
}

function schedulePlatformRecovery(taskId: string, delayMs = PLATFORM_RECOVERY_POLL_MS) {
  if (platformRecoveryTimers.has(taskId)) return
  const timer = setTimeout(() => {
    platformRecoveryTimers.delete(taskId)
    recoverPlatformTask(taskId)
  }, delayMs)
  platformRecoveryTimers.set(taskId, timer)
}
...
async function completeRecoveredPlatformTask(task: TaskRecord, result: Awaited<ReturnType<typeof normalizePlatformGenerationJob>>) {
  const latest = useStore.getState().tasks.find((item) => item.id === task.id)
  if (!latest || latest.status === 'done') return
  ...
  updateTaskInStore(task.id, {
    ...
    costCredits: result.creditsQuoted ?? task.costCredits,
    creditsCharged: result.creditsCharged ?? task.creditsCharged,
  ...
  useStore.getState().showToast(`平台任务已完成，共 ${outputIds.length} 张图片`, 'success')
  if (!isAgentTask(task)) showTaskCompletionNotification('图像生成完成', `平台任务已完成，共 ${outputIds.length} 张图片。`)
}

async function recoverPlatformTask(taskId: string) {
  const { settings, tasks } = useStore.getState()
  const task = tasks.find((item) => item.id === taskId)
  if (!task || task.apiProvider !== 'platform' || !task.platformJobId || task.status === 'done') return

  const profile = getPlatformRecoveryProfile(settings, task)
  try {
    const { job } = await getPlatformGeneration(profile, task.platformJobId)
    if (job.status === 'queued' || job.status === 'running') {
      schedulePlatformRecovery(taskId)
      return
    }

    clearPlatformRecoveryTimer(taskId)
    if (job.status === 'succeeded') {
      const mime = MIME_MAP[task.params.output_format] || 'image/png'
      const result = await normalizePlatformGenerationJob(profile, job, mime, task.costCredits, task.creditsCharged)
      await completeRecoveredPlatformTask(task, result)
      return
    }

    updateTaskInStore(taskId, {
      status: 'error',
      error: job.status === 'cancelled' ? '任务已取消' : toUserFacingGenerationError(job.errorMessage),
      platformRecoverable: false,
      finishedAt: Date.now(),
      elapsed: Date.now() - task.createdAt,
    })
...
export async function initStore() {
  ...
  for (const task of tasks) {
    ...
    if (
      task.apiProvider === 'platform' &&
      task.platformJobId &&
      (task.status === 'running' || task.platformRecoverable)
    ) {
      schedulePlatformRecovery(task.id, 0)
    }
  }
...
async function executeTask(taskId: string) {
  ...
  try {
    ...
    updateTaskInStore(taskId, {
      ...
      costCredits: result.creditsQuoted,
    ...
    } else if (err instanceof PlatformGenerationPendingError) {
      updateTaskInStore(taskId, {
        status: 'running',
        error: null,
        platformJobId: err.jobId,
        platformRecoverable: true,
        costCredits: err.creditsQuoted ?? latestTask.costCredits,
        creditsCharged: err.creditsCharged ?? latestTask.creditsCharged,
        falRecoverable: false,
        customRecoverable: false,
        finishedAt: null,
        elapsed: null,
      })
      useStore.getState().showToast('生成时间较长，系统会继续查询结果，完成后自动显示图片。', 'info')
      schedulePlatformRecovery(taskId, 0)
    } else {

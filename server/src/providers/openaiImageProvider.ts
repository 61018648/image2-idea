...
function isChineseTextSensitive(prompt: string): boolean {
  return /[\u3400-\u9fff]/.test(prompt)
    && /(文字|汉字|中文|字体|标题|海报|招牌|标语|书法|手写|排版|logo|LOGO|字|词|句|文案|印刷)/i.test(prompt)
}

function enhancePromptForChineseText(prompt: string): string {
  if (!isChineseTextSensitive(prompt)) return prompt
  return [
    '请严格保留所有中文文字的字形、顺序和含义，不要改写、增删、翻译或替换任何汉字。',
    '如果提示词中有引号、冒号后面的指定文字、标题、招牌或海报文案，必须逐字一致；复杂汉字按原字逐笔画生成，文字需要清晰可读。',
    prompt,
  ].join('\n')
}

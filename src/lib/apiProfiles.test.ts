...
describe('validateApiProfile', () => {
  ...
  it('allows platform profiles without API URL or API Key', () => {
    expect(validateApiProfile(createDefaultPlatformProfile({
      baseUrl: '',
      apiKey: '',
...
\`\`\``)
  ...
  it('normalizes platform profiles with empty API URL and API Key', () => {
    const settings = normalizeSettings({
      profiles: [{
        id: 'platform-profile',
        name: '平台托管',
        provider: 'platform',
        baseUrl: '',
        apiKey: 'secret-should-be-dropped',
        model: DEFAULT_PLATFORM_MODEL,
        timeout: 120,
        apiMode: 'images',
        codexCli: false,
    ...
    expect(getActiveApiProfile(settings)).toMatchObject({
      provider: 'platform',
      apiKey: '',
      model: DEFAULT_PLATFORM_MODEL,
    })
  ...
  it('keeps non-OpenAI providers in Images API mode when switching providers', () => {
    ...
    const platformProfile = switchApiProfileProvider(openaiProfile, 'platform')
    ...
    expect(platformProfile).toMatchObject({ provider: 'platform', apiMode: 'images', streamImages: false, apiKey: '', model: DEFAULT_PLATFORM_MODEL })
  })
